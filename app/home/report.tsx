...
const FormSection = ({ activeField, unitValue, reportType, formData, setFormData }) => {
    ...
    const getExpirationDate = (months) => {
        ...
        expiration.setMonth(expiration.getMonth() + (months ?? 0));
            ...
            {(!activeField || activeField === 'category') && (
                <ScrollView contentContainerStyle={{ paddingBottom: 150 }}>
                    ...
                    {PET_OPTIONS.map((option) => (
                        <TouchableOpacity
                            key={option.value}
                            className={`mb-4 px-5 py-6 rounded-3xl ${option.value === formData.category ? 'bg-gray-100 border-gray-300' : 'bg-white border-gray-200'} border`}
                ...
                <View>
                    ...
                    <View className="mb-4">
                        ...
                        <View className="flex-row items-center mt-2 justify-end">
                            <Text className="text-right text-sm text-gray-500">{formData.title.length}/30</Text>
                        </View>
                    </View>
                </View>
            )}
            {(!activeField || activeField === 'details') && (
                <View className="flex-1">
                    ...
                    <View className="flex-1 mb-4">
                        ...
                        />
                        <View className="flex-row items-center mt-2 justify-end">
                            <Text className="text-right text-sm text-gray-500">{formData.details.length}/500</Text>
                        </View>
                    </View>
                </View>
            )}
...
const ImageUploadSection = ({ selectedImages, setSelectedImages }) => {
        ...
        <View className="p-4">
            ...
            <View className="flex-row flex-wrap">
                ...
                {selectedImages.length < 5 && (
                    <TouchableOpacity
                        className={`${selectedImages.length === 0 ? 'w-full p-1' : 'w-1/2 p-1'}`}
                        onPress={() => {
...
export default function ReportScreen() {
    ...
    const renderStep = () => {
        ...
        switch (step.name) {
            case 'images':
                return <ImageUploadSection selectedImages={selectedImages} setSelectedImages={setSelectedImages} />;
            default:
                return (
                ...
                <View className="px-6 pb-8 bg-white">
                    <View className="flex-row">
                        ...
                        {currentStep < filteredSteps.length - 1 ? (
                            <TouchableOpacity
                                className={`py-4 rounded-xl bg-orange-500 ${currentStep === 0 ? 'flex-1' : 'flex-[8]'}`}
                                onPress={handleNext}
                            >
