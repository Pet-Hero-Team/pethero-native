...
// ⭐️⭐️⭐️ 새로운 DB 구조에 맞춰 데이터 조회 함수를 모두 수정합니다. ⭐️⭐️⭐️

const fetchQuestion = async (id: string) => {
    const { data, error } = await supabase
        .from('pet_questions')
        .select(`
            id, user_id, title, description, animal_type, created_at,
            pet_question_images (url),
            pet_question_categories (
                disease_categories ( tag_value, name )
            )
        `)
        .eq('id', id)
        .single();
    ...
    const categoryInfo = data.pet_question_categories?.[0]?.disease_categories;

    return {
        ...data,
        images: data.pet_question_images || [],
        disease_tag: categoryInfo || { name: '미지정', tag_value: 'etc' }
    };
};
...
const fetchRelatedQuestions = async (diseaseTagValue: string, currentQuestionId: string) => {
    if (!diseaseTagValue || diseaseTagValue === 'etc') return [];

    const { data, error } = await supabase
        .from('pet_questions')
        .select(`
            id, title, description, animal_type, created_at,
            pet_question_images (url),
            pet_question_categories!inner (
                disease_categories!inner (tag_value, name)
            )
        `)
        .eq('pet_question_categories.disease_categories.tag_value', diseaseTagValue)
...
export default function QuestionsDetailScreen() {
    ...
    const { data: relatedQuestions = [], isLoading: isRelatedLoading } = useQuery({
        queryKey: ['relatedQuestions', question?.disease_tag.tag_value, id],
        queryFn: () => fetchRelatedQuestions(question?.disease_tag.tag_value || 'etc', id as string),
        enabled: !!question && !!id,
    });
    ...
    const deleteQuestionMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: ['questions'] }); // 질문 목록 캐시 무효화
            router.back();
        },
        onError: (error) => {
    ...
    if (questionError || !question) {
        return (
            <SafeAreaView className="flex-1 bg-white justify-center items-center p-4">
                <Text className="text-lg font-semibold text-gray-800 text-center">질문을 찾을 수 없거나 불러오는 데 실패했습니다.</Text>
                <Text className="text-red-500 mt-2">{questionError?.message}</Text>
                <Pressable className="bg-teal-500 py-3 rounded-xl mt-6 px-6" onPress={() => router.back()}>
    ...
    const seeMoreButtonText = `${getTreatmentLabel(question.disease_tag.name)} 관련 더보기`;
            ...
            <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
                <ScrollView className="flex-1 bg-neutral-100">
                    ...
                    <View className="pt-8 bg-white">
                        <View className="bg-white rounded-lg px-6 pb-8">
                            ...
                            <View className="flex-row items-center pt-4">
                                ...
                                <View className="bg-neutral-100 py-2 px-3 rounded-lg ml-2"><Text className="text-sm font-semibold text-neutral-600">{question.disease_tag.name}</Text></View>
                            </View>
                        </View>
                    </View>
                    <ShadowViewLight className="bg-white">
                    ...
                    <View className="mt-6 bg-white py-12">
                        <Text className="text-2xl font-semibold text-neutral-800 px-6">{seeMoreButtonText}</Text>
                        {isRelatedLoading ? (<View>{Array.from({ length: 3 }).map((_, index) => (<RelatedQuestionSkeleton key={index} />))}</View>) : relatedQuestions.length === 0 ? (<View className="flex-1 justify-center items-center py-20"><Text className="text-neutral-600 text-center">유사 질문이 없습니다.</Text></View>) : (
                        ...
                        {relatedQuestions.length > 0 && (
                            <Link href={{ pathname: "/medical/questions/questions", params: { selectedTreatmentTag: question.disease_tag.tag_value } }} asChild>
                                <Pressable>
